...
export function chooseSequence(style: string, nAgents: number, nPicks: number, seed?: number): number[] {
...
export function analyzeUtilities(allocation: Allocation, preferences: Preferences): UtilityStats {
  ...
  for (const agent in allocation) {
    totalUtilities[agent] = allocation[agent].reduce((sum, obj) => sum + preferences[agent][obj], 0);
  }
...
export function calculateEnvy(allocation: Allocation, preferences: Preferences): { envyValue: number; envyMatrix: EnvyMatrix } {
