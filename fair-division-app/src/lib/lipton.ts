...
import { pool } from '$lib/pool'; // Importation du store pool
import { get } from 'svelte/store'; // Importation de la fonction get pour accéder au store
...
export function liptonAllocate(prefs: Preferences): Allocation {
	...
	// Récupérer les objets depuis le store pool
	const poolData: Record<string, number> = get(pool); // Typage explicite pour poolData
	const objects = Object.keys(poolData).flatMap((color) =>
		Array(poolData[color] || 0).fill(color)
	); // Convertir le pool en une liste d'objets
